...
export const authService = {
  ...
  signInWithGoogle: async () => {
    try {
      ...
      throw new Error('Google sign-in failed to retrieve session');
    } catch (error: any) {
      // Gracefully handle cancellation errors without triggering app-level logging errors
      const isCancellation = 
        error?.message === 'Google sign-in was cancelled' || 
        error?.message === 'Google sign-in failed to retrieve session';
        
      if (isCancellation) {
        console.log('[AuthService] Google sign-in was cancelled by the user.');
      } else {
